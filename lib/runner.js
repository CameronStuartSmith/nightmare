 ...
 */
 ...
 // Determine if we should mock a process
 const shouldMock = typeof process.send === 'undefined'

 // var parent = require('./ipc')(process)
 var _parent = shouldMock ? null : require('./ipc')(process);
 ...
 process.on('uncaughtException', function(err) {
   ...
   if (!shouldMock) _parent.emit('uncaughtException', e.stack)
 })
 ...
 if (!shouldMock) {
	  ...
	  * Listen for the app being "ready"
	  */
	 app.on('ready', appReady.bind(undefined, _parent))
 }
 
 function appReady(parent) {
	 console.log('Calling APP READY');
	 console.log('Parent', parent._IS_IPC)
	 if (!parent._IS_IPC){
		 parent = require('./ipc')(parent);
	 }
   ...
   parent.respondTo('quit', function(done) {
	 if (!shouldMock) app.quit();
	 else {
		 win.destroy();
		 parent.process.emit('close');
	 }
	 done()
   })
